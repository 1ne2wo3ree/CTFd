...
Alpine.data("ChallengeBoard", () => ({
  ...
  applyTierGating() {
    // Automatic Level / Tier Gating Engine
    if (window.TACTICAL_TIER_GATING === false) return;

    const easyChals = this.challenges.filter(c => c.value > 0 && c.value < 200 && c.name !== "???");
    const mediumChals = this.challenges.filter(c => c.value >= 200 && c.value <= 350 && c.name !== "???");
    const hardChals = this.challenges.filter(c => c.value > 350 && c.value <= 500 && c.name !== "???");

    const allEasySolved = easyChals.length === 0 || easyChals.every(c => c.solved_by_me);
    const allMediumSolved = mediumChals.length === 0 || mediumChals.every(c => c.solved_by_me);
    const allHardSolved = hardChals.length === 0 || hardChals.every(c => c.solved_by_me);

    this.challenges.forEach(c => {
      c.tier_locked = false;
      c.lock_reason = "";

      if (c.name !== "???" && !c.solved_by_me) {
        if (c.value >= 200 && c.value <= 350 && !allEasySolved) {
          c.tier_locked = true;
          c.lock_reason = "SOLVE ALL EASY MISSIONS FIRST";
        } else if (c.value > 350 && c.value <= 500 && (!allEasySolved || !allMediumSolved)) {
          c.tier_locked = true;
          c.lock_reason = "SOLVE ALL MEDIUM MISSIONS FIRST";
        } else if (c.value > 500 && (!allEasySolved || !allMediumSolved || !allHardSolved)) {
          c.tier_locked = true;
          c.lock_reason = "SOLVE ALL HARD MISSIONS FIRST";
        }
      }
    });
  },
